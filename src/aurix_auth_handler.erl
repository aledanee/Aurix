            ...
            case validate_email(Email) of
                ok ->
                    case aurix_auth_service:register(TenantCode, Email, Password) of
                        {ok, Result} ->
                            reply_json(201, Result, Req1);
                        {error, Reason} ->
                            reply_service_error(Reason, Req1)
                    end;
                error ->
                    aurix_auth_middleware:reply_error(400, <<"invalid_email">>, <<"Invalid email format">>, Req1)
            end;
        _ ->
            ...
            case jsx:decode(Body, [return_maps]) of
                #{<<"refresh_token">> := RefreshToken} ->
                    case aurix_auth_service:logout(RefreshToken) of
                        ok ->
                            reply_json(200, #{<<"message">> => <<"Logged out successfully">>}, Req1);
                        {error, Reason} ->
                            reply_service_error(Reason, Req1)
                    end;
                _ ->
